...
{
  static struct gcov_summary prg = {0}; /* summary for this object over all program.  */
  int error;
#ifndef ENABLE_LIBGCOV_PORT
  gcov_unsigned_t tag;
#endif
...
static struct gcov_summary real_all_prg = {0};
static struct gcov_summary real_this_prg = {0};
...
*/
  ...
  crc32 = compute_summary (list, &real_this_prg /* @NO_GCDA_FILE , &gf.max_length*/);
...
#if !GCOV_LOCKED
  memset (&real_all_prg, 0, sizeof (real_all_prg));
#endif
...
*/
  for (gi_ptr = list; gi_ptr && !(gi_ptr == list && break_loop_counter); gi_ptr = gi_ptr->next, break_loop_counter++)
    dump_one_gcov (gi_ptr, /* @NO_GCDA_FILE &gf,*/ run_counted, crc32, &real_all_prg, &real_this_prg);
